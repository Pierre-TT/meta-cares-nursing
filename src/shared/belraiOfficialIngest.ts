import type { Json, Tables, TablesInsert } from '../lib/database.types.ts';
...
type BelraiCapInsert = TablesInsert<'belrai_caps'>;
type BelraiScoreInsert = TablesInsert<'belrai_scores'>;
...
export async function ingestBelraiOfficialResultRecord(
  ...
  const { data: officialAssessment, error: officialAssessmentError }: BelraiQueryResult<BelraiAssessmentRow> = await belraiTable<BelraiAssessmentRow>(client, 'belrai_assessments')
    .upsert({
      ...
      linked_prep_assessment_id: prepAssessment?.id ?? existingOfficialAssessment?.linked_prep_assessment_id ?? null,
      external_assessment_id: payload.externalAssessmentId ?? existingOfficialAssessment?.external_assessment_id ?? null,
      record_role: 'official',
      template_key: payload.templateKey ?? prepAssessment?.template_key ?? existingOfficialAssessment?.template_key ?? 'interrai_hc_screener',
      template_version: payload.templateVersion ?? prepAssessment?.template_version ?? existingOfficialAssessment?.template_version ?? 'official-v1',
      assessment_scope: payload.assessmentScope ?? prepAssessment?.assessment_scope ?? existingOfficialAssessment?.assessment_scope ?? 'screening',
      status: 'synced',
      sync_status: 'synced',
      source: sourceSystem,
      ...
      shared_with_patient_at: payload.sharedWithPatientAt ?? existingOfficialAssessment?.shared_with_patient_at ?? null,
      last_synced_at: receivedAt,
      completed_at: receivedAt,
      submitted_at: prepAssessment?.submitted_at ?? existingOfficialAssessment?.submitted_at ?? receivedAt,
    }, { onConflict: 'id' })
    .select('*')
    .single();
