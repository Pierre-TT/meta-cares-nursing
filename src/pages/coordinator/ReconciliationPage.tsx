import { Landmark, Download, ArrowRightLeft } from 'lucide-react';
...
const payments: Payment[] = [
  { id: '1', date: '05/03/2025', reference: 'VIRT-MC200-2025-0312', mutuality: 'MC 200', amount: 4523.80, matchedInvoices: 42, status: 'matched' },
  { id: '2', date: '04/03/2025', reference: 'VIRT-MC400-2025-0287', mutuality: 'MC 400', amount: 2187.50, matchedInvoices: 18, status: 'matched' },
  { id: '3', date: '03/03/2025', reference: 'VIRT-MC300-2025-0195', mutuality: 'MC 300', amount: 1892.30, matchedInvoices: 15, status: 'partial' },
  { id: '4', date: '01/03/2025', reference: 'VIRT-CAAMI-2025-0044', mutuality: 'CAAMI 600', amount: 345.20, matchedInvoices: 0, status: 'unmatched' },
];
...
export function ReconciliationPage() {
  ...
  const matchedAmount = payments.filter(p => p.status === 'matched').reduce((s, p) => s + p.amount, 0);
            ...
            <div className="flex items-start justify-between">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <p className="font-semibold text-sm">{p.mutuality}</p>
                </div>
                <p className="text-xs text-[var(--text-muted)] mt-0.5">{p.date} · Réf: {p.reference}</p>
                {p.matchedInvoices > 0 && (
                  <p className="text-xs text-[var(--text-muted)]">{p.matchedInvoices} factures rapprochées</p>
                )}
              </div>
    ...
    <div className="space-y-3">
      <Card>
        ...
        <div className="space-y-2 text-sm">
          ...
          <div className="flex justify-between"><span className="text-[var(--text-muted)]">En attente de paiement</span><span className="font-bold text-mc-amber-500">€{(12845.60 - totalReceived).toFixed(2)}</span></div>
      ...
      <Button variant="outline" className="w-full gap-2">
        <Download className="h-4 w-4" /> Exporter rapprochement (CSV)
      </Button>
    </div>
  );
    ...
    <AnimatedPage className="px-4 py-6 lg:px-8 max-w-5xl mx-auto space-y-4">
      ...
      >
        <div className="flex items-center justify-around mt-1">
          ...
          <div className="h-6 w-px bg-white/20" />
          <div className="text-center">
            <p className="text-lg font-bold text-white">{payments.filter(p => p.status === 'unmatched').length}</p>
